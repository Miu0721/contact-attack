...
`.trim();
  ...
  try {
    if (response.output_text) {
      ...
      // Responses API の標準構造からテキストを取り出す fallback
    ...
    console.warn('AI レスポンスのテキスト抽出に失敗しました:', e);
  }
  ...
  // ▼ 余計な文字が混ざっても、{} の部分だけ抜き取って JSON.parse する
  let jsonStr = raw;
  ...
  try {
    ...
    console.warn('AI からの返答の JSON 解析に失敗しました:', jsonStr);
    return null;
  }
